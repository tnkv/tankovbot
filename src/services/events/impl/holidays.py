...
holidays_events = [
    ...
    ConditionEvent(
        name="Рейс 11 (Редкий)",
        condition=lambda u, dt: dt.month != 9 or dt.day != 11,
        chance_percent=0.05,
    ...
    CalendarEvent(
        ...
        text="🏹 Купидон стрелял в сердце, но попал чуть ниже... Твой размер изменился на 14 см!"
    ),
    CalendarEvent(
        name="День дурака",
        month=4, day=1, chance_percent=100.0, action=0,
        text="🃏 ОГО! НЕВЕРОЯТНЫЙ КРИТ! +100 см!... А, нет, с 1 апреля. Ничего не изменилось."
    ),
    CalendarEvent(
        name="Чернобыльская мутация",
        month=4, day=26, chance_percent=5.0, action=86,
        text="📻 Дозиметр зашкаливает, 3.6 рентген! Третья нога выросла на славу. Радиация дарит +86 см!"
    ),
    CalendarEvent(
        name="Костры рябин",
        month=9, day=3, chance_percent=30.0,
    ...
    CalendarEvent(
        ...
        month=3, day=17, chance_percent=20.0,
        action=lambda u, bg: 15, # Simplified to 15
        text="🍺 Пьяный лепрекон блеванул тебе в штаны! Сегодня только зеленые плюсы!"
    ),
    CalendarEvent(
        name="Мужской день",
        month=2, day=23, chance_percent=15.0, action=23,
        text="🧦 Носки и пена для бритья — это база, но держи +23 см за защиту отечества."
    ),
    CalendarEvent(
        name="Женский день",
        month=3, day=8, chance_percent=80.0, action=-8,
        text="🎀 Пришлось отрезать кусок на подарок любимой. Минус 8 см на благое дело."
    ),
    CalendarEvent(
        name="Поехали!",
        month=4, day=12, chance_percent=10.0,
    ...
    ConditionEvent(
        ...
        chance_percent=40.0, action=-13,
        text="🔪 Джейсон Вурхиз отмахнул тебе мачете лишнее. Проклятые -13 см."
    ),
    ConditionEvent(
        name="Черная пятница",
    ...
    CalendarEvent(
        name="Демократия",
        month=7, day=4, chance_percent=100.0, action=4,
        text="🦅 СВОБОДА! ДЕМОКРАТИЯ! И +4 дюйма от дядюшки Сэма."
    ),
    ConditionEvent(
        name="Октоберфест",
        condition=lambda u, dt: dt.month == 9,
        chance_percent=1.0, action=15,
        text="🥨 Пивное пузо давит, но агрегат набух! +15 см сегодня (жди похмелья завтра)."
    ),
    CalendarEvent(
        name="День знаний",
        month=9, day=1, chance_percent=100.0, action=1,
        text="🧑‍🎓 Мал золотник, да дорог. Держи +1 см за выученный стих у доски."
    ),
    CalendarEvent(
        name="Мир, Труд, Май",
        month=5, day=1, chance_percent=40.0, action=15,
        text="🛠 Ударно потрудился на даче, поднимая целину! +15 см от профсоюза."
    ),
    ConditionEvent(
    ...
    CalendarEvent(
        name="Високосный",
        month=2, day=29, chance_percent=15.0, action=29,
        text="⏳ Такое бывает раз в 4 года! Забирай свои редкие +29 см."
    )
]
