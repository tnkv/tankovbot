...
def wait_msg(time_seconds: int) -> str:
    ...
    return f"😐 Вы уже крутили кок сегодня.\n✅ До следующего кручения: {t}"

def spam_penalty_msg(penalty: int) -> str:
    return f"🤬 Вы доигрались! За спам командой Ваш кок оштрафован на {penalty} см! (Стояк сохранен)"

def profile_msg(db_reg_date: int, db_cock_length: int, db_last_cock: int, db_old_cock: int, db_streak: int) -> str:
    try:
